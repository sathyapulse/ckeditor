...
CKEDITOR.dialog.add('specialchar', k => {
  var e,
    ...
    g = function(c, b) {
      ...
      b.getName() == 'span' && (b = b.getParent());
      if (b.getName() == 'a' && (a = b.getChild(0).getHtml())) {
        l && d(null, l);
        const f = e.getContentElement('info', 'htmlPreview').getElement();
        e
    ...
    onLoad() {
      for (
        var c = this.definition.charColumns,
          ...
          f = [
            `\x3ctable role\x3d"listbox" aria-labelledby\x3d"${a}" style\x3d"width: 320px; height: 100%; border-collapse: separate;" align\x3d"center" cellspacing\x3d"2" cellpadding\x3d"2" border\x3d"0"\x3e`,
          ],
          d = 0,
          g = b.length,
          h,
          ...
          if ((h = b[d])) {
            h instanceof Array
              ...
              : ((e = h
                  ...
                  .replace('#', '')),
                (e = n[e] || h));
            const m = `cke_specialchar_label_${d}_${CKEDITOR.tools.getNextNumber()}`;
            f.push(
              `\x3ctd class\x3d"cke_dark_background" style\x3d"cursor: default" role\x3d"presentation"\x3e\x3ca href\x3d"javascript: void(0);" role\x3d"option" aria-posinset\x3d"${d +
                1}"`,
              ` aria-setsize\x3d"${g}"`,
      ...
      f.push(
        ...
        `\x3cspan id\x3d"${a}" class\x3d"cke_voice_label"\x3e${
          n.options
        }\x3c/span\x3e`,
      );
            ...
            children: [
              {
                ...
                onShow() {
                  const c = this.getElement().getChild([0, 0, 0, 0, 0]);
                  setTimeout(() => {
                    c.focus();
                    g(null, c);
                  }, 0);
