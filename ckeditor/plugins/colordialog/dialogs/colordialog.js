...
CKEDITOR.dialog.add('colordialog', x => {
  ...
  function D(a) {
    ...
    return 0.2126 * b[0] + 0.7152 * b[1] + 0.0722 * b[2] >= 165;
  }
  function A(a) {
    !a.name && (a = new CKEDITOR.event(a));
    let c = !/mouse/.test(a.name),
      b = a.data.getTarget(),
      k;
    b.getName() == 'td' &&
  ...
  function E(a) {
    ...
    switch (k) {
      ...
      case d ? 37 : 39:
        (a = b.getNext())
          ? a.type == 1 && (a.focus(), c.preventDefault(!0))
          : (a = b.getParent().getNext()) &&
            (a = a.getChild([0])) &&
            a.type == 1 &&
  ...
  (function() {
    function a(a, d) {
      for (let w = a; w < a + 3; w++) {
        const e = new v(h.$.insertRow(-1));
        e.setAttribute('role', 'row');
        for (let f = d; f < d + 3; f++)
          for (let g = 0; g < 6; g++) c(e.$, `#${b[f]}${b[g]}${b[w]}`);
      }
    }
    function c(a, c) {
      const b = new v(a.insertCell(-1));
                ...
                children: [
                  {
                    ...
                    html: `\x3cspan\x3e${
                      g.highlight
                    }\x3c/span\x3e\x3cdiv id\x3d"${t}" style\x3d"border: 1px solid; height: 74px; width: 74px;"\x3e\x3c/div\x3e\x3cdiv id\x3d"${u}"\x3e\x26nbsp;\x3c/div\x3e\x3cspan\x3e${
                      g.selected
                    }\x3c/span\x3e\x3cdiv id\x3d"${n}" style\x3d"border: 1px solid; height: 20px; width: 74px;"\x3e\x3c/div\x3e`,
                  },
                  {
                    type: 'text',
                    label: g.selected,
