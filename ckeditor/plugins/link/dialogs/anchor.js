...
CKEDITOR.dialog.add('anchor', c => {
  ...
  return {
    ...
    minHeight: 60,
    onOk() {
      let b = CKEDITOR.tools.trim(this.getValueOf('info', 'txtName')),
    ...
    onShow() {
      let b = c.getSelection(),
        a;
      a = b.getRanges()[0];
      let d = b.getSelectedElement();
      a.shrink(CKEDITOR.SHRINK_ELEMENT);
      a =
        (d = a.getEnclosedNode()) &&
        d.type === CKEDITOR.NODE_ELEMENT &&
        (d.data('cke-real-element-type') === 'anchor' || d.is('a'))
          ? d
          : void 0;
      const f = (d = a && a.data('cke-realelement'))
        ? CKEDITOR.plugins.link.tryRestoreFakeAnchor(c, a)
        : CKEDITOR.plugins.link.getSelectedLink(c);
      if (f) {
        this._.selectedElement = f;
        const e = f.data('cke-saved-name');
