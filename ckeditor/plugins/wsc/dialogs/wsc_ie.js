...
CKEDITOR.dialog.add('checkspell', a => {
  function c(a, c) {
    let d = 0;
    return function() {
      typeof window.doSpell === 'function'
        ? (typeof e !== 'undefined' && window.clearInterval(e), l(a))
        : d++ == 180 && window._cancelOnError(c);
    };
  }
  function l(c) {
    let f = new window._SP_FCK_LangCompare(),
  ...
  var b = CKEDITOR.tools.getNextNumber(),
    ...
    n =
      ...
      `${b}//loader.webspellchecker.net/sproxy_fck/sproxy.php?plugin\x3dfck2\x26customerid\x3d${
        a.config.wsc_customerId
      }\x26cmd\x3dscript\x26doc\x3dwsc\x26schema\x3d22`;
  a.config.wsc_customLoaderScript &&
    (k += `\x3cp style\x3d"color:#000;font-size:11px;font-weight: normal;text-align:center;padding-top:10px"\x3e${a.lang.wsc.errorLoading.replace(
      /%s/g,
      a.config.wsc_customLoaderScript,
    )}\x3c/p\x3e`);
  window._cancelOnError = function(c) {
    if (typeof window.WSC_Error === 'undefined') {
      ...
      const b = CKEDITOR.document.getById(h);
  ...
  return {
    ...
    onShow() {
      let b = this.getContentElement('general', 'content').getElement();
      b.setHtml(m);
