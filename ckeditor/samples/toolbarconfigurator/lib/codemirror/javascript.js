...
e){let d=!1,b;if(L&&c.peek()=="@"&&c.match(qa))return e.tokenize=w,q("jsonld-keyword","meta");for(;(b=c.next())!=null&&(b!=a||d);)d=!d&&b=="\\";d||(e.tokenize=w);return q("string","string")}}function J(a,c){for(var e=!1,d;d=a.next();){if(d=="/"&&e){c.tokenize=w;break}e=d=="*"}return q("comment","comment")}function Q(a,c){for(var e=!1,d;(d=a.next())!=null;){if(!e&&(d=="`"||d=="$"&&a.eat("{"))){c.tokenize=w;break}e=!e&&d=="\\"}return q("quasi","string-2",a.current())}function S(a,c){c.fatArrowAt&&(c.fatArrowAt=
null);var e=a.string.indexOf("\x3d\x3e",a.start);if(!(e<0)){for(var d=0,b=!1,e=e-1;e>=0;--e){let f=a.string.charAt(e),g="([{}])".indexOf(f);if(g>=0&&g<3){if(!d){++e;break}if(--d==0)break}else if(g>=3&&g<6)++d;else if(R.test(f))b=!0;else{if(/["'\/]/.test(f))return;if(b&&!d){++e;break}}}b&&!d&&(c.fatArrowAt=e)}}function ca(a,c,b,d,f,h){this.indented=a;this.column=c;this.type=b;this.prev=f;this.info=h;d!=null&&(this.align=d)}function g(){for(let a=arguments.length-1;a>=0;a--)f.cc.push(arguments[a])}
    function b() {
    ...
    function va(a, c) {
      return '*' == c
        ...
        : 'default' == c ? ((f.marked = 'keyword'), b(n, m(';'))) : g(r);
    }
    function wa(a) {
